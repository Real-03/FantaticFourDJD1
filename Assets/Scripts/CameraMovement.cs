...
public class CameraMovement : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        // Ajusta o zoom com base na distância entre os jogadores
        float targetZoom = Mathf.Lerp(minZoom, maxZoom, distance / maxDistance);
        cam.orthographicSize = Mathf.Lerp(cam.orthographicSize, targetZoom, Time.deltaTime * zoomSpeed);

        // Para de mover a câmera se os jogadores estiverem no limite do zoom e correndo para fora
        if (distance >= maxDistance * 0.95f && ArePlayersMovingApart())
        {
            return;
        }
        
        // Move a câmera suavemente para o ponto médio
    ...
    bool ArePlayersMovingApart()
    {
        ...
        return Vector2.Dot(dir1, dir2) < -0.5f; // Valores próximos de -1 indicam direções opostas
    }
}
